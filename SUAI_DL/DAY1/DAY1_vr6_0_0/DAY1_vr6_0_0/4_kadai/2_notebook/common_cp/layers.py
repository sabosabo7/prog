...
class BatchNormalization:
    ...
    def __forward(self, x, train_flg, epsilon=1e-8):
        ...
        if (self.moving_mean is None) or (self.moving_var is None):
            N, D = x.shape
            self.moving_mean = cp.zeros(D)
            self.moving_var = cp.zeros(D)
        ...
        if train_flg:
            ...
            # 入力xの分散を求める
            var = cp.mean(x_mu**2, axis=0)  # 要素数d個のベクトル
            
            # 入力xの標準偏差を求める(epsilonを足してから標準偏差を求める)
            std = cp.sqrt(var + epsilon)  # 要素数d個のベクトル
            ...
            x_std = x_mu / cp.sqrt(self.moving_var + epsilon) # n*d行列
    ...
    def __backward(self, dout):
        ...
        # gammaの勾配(n方向に合計)
        dgamma = cp.sum(self.x_std * dout, axis=0)
        ...
        # 標準偏差の勾配
        a4 = -(cp.sum(a3, axis=0) ) / (self.std * self.std)
        ...
        # Xの勾配
        dx = a2 + a7 +  cp.sum(a8, axis=0) / self.batch_size # 第3項はn方向に平均
...
class MaxPooling:
    ...
    def forward(self, x,train_flg=True):
        ...
        col = im2col(x, self.pool_h, self.pool_w, self.stride, self.pad, constant_values=-cp.inf)
    ...
    def backward(self, dout):
        ...
        dcol = cp.zeros((dout.size, pool_size))
        
        # 順伝播計算時に最大値となった場所に、doutを配置する
        # dout.flatten()はdoutを1次元配列に変換している
        dcol[cp.arange(dcol.shape[0]), self.arg_max] = dout.flatten()
