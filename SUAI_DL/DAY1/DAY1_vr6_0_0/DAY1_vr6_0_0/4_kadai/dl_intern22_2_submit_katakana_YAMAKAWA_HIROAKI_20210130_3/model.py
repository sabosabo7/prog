...
class DeepConvNet:
    def __init__(self, input_dim=(1, 28, 28),             
                 hidden_size=100, output_size=15, weight_init_std=0.01,param=None,const={"Batch1":[None,None],"Batch2":[None,None],"Batch3":[None,None],"Batch4":[None,None],"Batch5":[None,None]}):
        """
        input_size : tuple, 入力の配列形状(チャンネル数、画像の高さ、画像の幅)
        conv_param : dict, 畳み込みの条件
        pool_param : dict, プーリングの条件
        ...
        if param==None:
            ...
            self.params['W3'] = std * np.random.randn(conv3_param['filter_num'], conv3_param["input_dim"], conv3_param['filter_size'], conv3_param['filter_size']) # W1は畳み込みフィルターの重みになる
            self.params['b3'] = np.zeros(conv3_param['filter_num']) #b1は畳み込みフィルターのバイアスになる

            self.params["gamma3"]=np.ones(batch3_param["input_dim"])
            self.params["beta3"]= np.zeros(batch3_param["input_dim"])

            self.params['W4'] = std * np.random.randn(conv4_param['filter_num'], conv4_param["input_dim"], conv4_param['filter_size'], conv4_param['filter_size']) # W1は畳み込みフィルターの重みになる
            self.params['b4'] = np.zeros(conv4_param['filter_num']) #b1は畳み込みフィルターのバイアスになる
            ...
            self.params['W5'] = std *  np.random.randn(np.prod(affin1_param["input_dim"]), hidden_size)
            ...
            self.params['W6'] = std *  np.random.randn(affin2_param["input_dim"], output_size)
